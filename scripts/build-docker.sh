...
# 第二个标签：默认同 git describe（含标签与提交距离）；CI 可 export IMAGE_VERSION_TAG=v1.2.3 覆盖
if [[ -z "${IMAGE_VERSION_TAG:-}" ]]; then
  IMAGE_VERSION_TAG="$(git describe --tags --always --dirty 2>/dev/null || echo "unknown")"
fi
# Docker 标签字符限制：简单去掉不合规字符
...
echo "Pushing tags: ${IMAGE_REPO}:latest ${IMAGE_REPO}:${IMAGE_VERSION_TAG}"
