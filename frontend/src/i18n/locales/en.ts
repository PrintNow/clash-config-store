const en = {
  ...
  customConfigs: {
    ...
    dialerProxyHint: 'Establish connection through specified proxy/group (proxy chain)',
    dialerProxyNone: 'Not set',
    dialerProxyNodeHint: 'Connect this node through the specified proxy/group instead of directly. E.g., relay your VPS through subscription nodes.',
    dialerProxyHelpTitle: 'What is Dialer Proxy?',
    dialerProxyHelpDesc: 'Dialer Proxy lets you connect to this node through another proxy server, forming a proxy chain.',
    dialerProxyFlowTitle: 'Current Connection Path',
    dialerProxyFlowClient: 'Client',
    dialerProxyFlowYou: 'Your Device',
    dialerProxyFlowRelay: 'Relay Proxy',
    dialerProxyFlowCurrentNode: 'Current Node',
    dialerProxyFlowTarget: 'Target',
    dialerProxyFlowWebsite: 'Website/Service',
    dialerProxyFlowConnect: 'connect',
    dialerProxyFlowForward: 'forward',
    dialerProxyFlowAccess: 'access',
    dialerProxyFlowDirect: 'direct',
    dialerProxyHelpHowItWorks: 'How it works',
    dialerProxyHelpExplanation: 'When Dialer Proxy is set, the client does not connect directly to this node. Instead, it first connects to the Dialer Proxy target (proxy/group), which then forwards traffic to this node. This adds an intermediate hop between the client and the target node.',
    dialerProxyHelpDiagramLine1: 'Client → Dialer Proxy (relay) → Current Node → Target Website',
    dialerProxyHelpDiagramLine2: 'Actual path: Core → Relay Server → Node Server → Target',
    dialerProxyHelpDiagramLine3: 'Effect: Target sees the node\'s IP; relay server sees you accessing the node',
    dialerProxyHelpUseCases: 'Typical use cases',
    dialerProxyHelpUseCase1: 'Relay subscription nodes through your own VPS for lower latency or to bypass restrictions',
    dialerProxyHelpUseCase2: 'Connect nodes through a specific SOCKS5/HTTP proxy (e.g., in isolated network environments)',
    dialerProxyHelpUseCase3: 'Multi-layer proxy chains for enhanced privacy (each layer only knows partial information)',
    rawYaml: 'Raw YAML',
  ...
  subscriptions: {
    ...
    dialerProxy: 'Dialer Proxy (Proxy Chain)',
    dialerProxyPlaceholder: 'Enter proxy node or group name',
    dialerProxyHint: 'When set, all provider proxy nodes in this subscription will connect through the specified proxy/group instead of directly. Used for acceleration or bypassing restrictions.',
