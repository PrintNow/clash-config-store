...
type CustomConfig struct {
	...
	Proxies []map[string]interface{} `gorm:"serializer:json;type:text" json:"proxies"`

	// ProxyGroups 代理组列表（JSON 序列化的 []map[string]interface{}）
	// 每项含 name/type/proxies/use 等字段
	ProxyGroups []map[string]interface{} `gorm:"serializer:json;type:text" json:"proxy_groups"`

	// Rules 规则行列表（JSON 序列化的 []string）
	Rules []string `gorm:"serializer:json;type:text" json:"rules"`

	// RuleProviderIDs 引用的规则集 ID 列表（JSON 序列化的 []uint）
	RuleProviderIDs []uint `gorm:"serializer:json;type:text" json:"rule_provider_ids"`
